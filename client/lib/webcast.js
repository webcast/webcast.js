...
(function() {
  ...
  Webcast.Encoder.Raw = (function() {
    ...
    Raw.prototype.encode = function(data) {
      var buf, chan, channels, i, samples, _i, _j, _ref, _ref1;
      channels = data.length;
      samples = data[0].length;
      buf = new Int8Array(channels * samples);
      for (chan = _i = 0, _ref = channels - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; chan = 0 <= _ref ? ++_i : --_i) {
        for (i = _j = 0, _ref1 = samples - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
  ...
  Webcast.Node = (function() {
    function Node(_arg) {
      ...
      this.node.onaudioprocess = function(buf) {
        var audio, channel, data, _i, _ref;
        audio = [];
        for (channel = _i = 0, _ref = _this.encoder.channels - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; channel = 0 <= _ref ? ++_i : --_i) {
